...
public:
    ...
    CubismMotionQueueEntryHandle Speak(const char* text, const char* soundPath, void (*onFinishedMotionHandler)(IMotion* self) = NULL);
    /**
     * @brief コンストラクタ
     */
    LAppModel();
     ...
     */
    CubismMotionQueueEntryHandle StartMotion(const char* group, signed int no, signed int priority, void (*onFinishedMotionHandler)(IMotion* self) = NULL);
     ...
     * @return                                  開始したモーションの識別番号を返す。個別のモーションが終了したか否かを判定するIsFinished()の引数で使用する。開始できない時は「-1」
     */
    CubismMotionQueueEntryHandle StartRandomMotion(const char* group, signed int priority, void (*onFinishedMotionHandler)(IMotion* self) = NULL);
